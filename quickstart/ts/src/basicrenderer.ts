...
export interface IStpRenderer {
    /**
     * Render symbol as a GeoJSON feature
     */
    asGeoJSON() : GeoJSON.Feature<any>;
    /**
     * 
     * @param size Render symbol as a geo-located svg
     */
    asSVG(): IGeoSvg[] | null;
}
...
export interface IGeoSvg {
     ...
     * 'icon': unit/equipment/mootw or single point TG non-scalable icon
     * 'label': decorations, such as TG identifiers
     * 'tg': geo-referenced (scalable) tactical graphics geometry
     */
    type: 'icon' | 'label' | 'tg';
    /**
     * Symbol title
    ...
    title?: string;
    /**
     * Top,Left coordinates (in decimal degrees)
     */
    topLeft: LatLon;
    /**
     * Bottom,Right coordinates (in decimal degrees)
     * Same as topLeft if feature is a Point
     */
    bottomRight: LatLon;
    /**
     * Coordinates where the symbol is placed
     */
    position: LatLon;
    /**
     * SVG content, bas64 encoded
     */
    svg: string;
    /**
     * Boundaries of the clickable region
     */
    shape: IPoint[];
    /**
    ...
    anchor: IPoint;
    /**
     * Optional additional properties, for example, label caption and styling
     */
    properties?: { [name: string]: any; } | null
}

type SvgGeom = 'rect' | 'circle' | 'ellipse' | 'line' | 'polyline' | 'polygon' | 'path' | 'text';

export class BasicRenderer implements IStpRenderer {
    symbol: StpSymbol;
    size: number;
     ...
     */
    constructor(symbol: StpSymbol, pointSize: number = 50) {
        this.symbol = symbol;
        this.size = pointSize;
    }
    ...
    asGeoJSON(): GeoJSON.Feature<any> {
        ...
        let res: GeoJSON.Feature<any> =  this.symbol.asGeoJSON();
        ...
        let gesture: GeoJSON.Geometry = this.getGestureGeometry(this.symbol);
        res.geometry = gesture;

        // Add SVG rendering, if applicable
        let svgRendering: IGeoSvg[] | null = this.asSVG();
        if (svgRendering) {
    ...
    asSVG(): IGeoSvg[] | null {
        if (this.symbol.location?.fsTYPE == 'point') {
            let gsvg: IGeoSvg[] = [ this.pointSVG(this.symbol) ];
            return gsvg;
        }
        else {
     ...
     */
    private getGestureGeometry(symbol: StpSymbol): GeoJSON.Geometry {
        ...
        let res: GeoJSON.Geometry;
        let geoType: "Point" | "LineString" | "Polygon" | "MultiPoint" | "MultiLineString" | "MultiPolygon" | "GeometryCollection";
        //let geoCoordinates: GeoJSON.Position[] ;
        let geoCoordinates: GeoJSON.Position | GeoJSON.Position[] | GeoJSON.Position[][];
        if (symbol.location.shape == "point") {
            ...
            if (symbol.location?.shape != null && symbol.location?.shape.includes("arrowfat")) {
                ...
                let reorderedLatLon: LatLon[] = symbol.location.coords.slice(0, symbol.location.coords.length - 1).reverse();
                ...
                let reorderedLatLon: LatLon[] = [ symbol.location.coords[0], symbol.location.coords[2], symbol.location.coords[3], symbol.location.coords[1] ];
                res = this.toLineString(reorderedLatLon);
                ...
                //     +--------->  
                //     4th?      2      
                let fourthPt: LatLon = new LatLon(symbol.location.coords[1].lat, symbol.location.coords[2].lon);
                let reorderedLatLon: LatLon[] = [ symbol.location.coords[0], symbol.location.coords[2], fourthPt, symbol.location.coords[1] ];
                res = this.toLineString(reorderedLatLon);
            } else if (symbol.location?.shape == "vee") {
                let reorderedLatLon: LatLon[] = [ symbol.location.coords[1], symbol.location.coords[0], symbol.location.coords[2] ];
                res = this.toLineString(reorderedLatLon);
     ...
     * @param symbol 
     */
    private pointSVG(symbol: StpSymbol) : IGeoSvg
    {
        ...
        if (symbol.affiliation === "friend") {
            ...
            let side: number = this.size / scaling - 2 * strokeWidth;
        ...
        let shape: IPoint[];
        if (rotation > 0) {
            const matrix: number[] = this.getTransform(svg, command);
            //[
     ...
     */
    private getTransform(svg: string, selector: SvgGeom): number[] {
        ...
        let matrix: DOMMatrix | null | undefined =  svgEl?.getCTM();
        // Clean up
     ...
     * @param coords 
     */
    private toLineString(coords: LatLon[]) {
        let res: GeoJSON.Geometry =  {
            type: "LineString",
     ...
     */
    private applyTransform(m: number[], point: IPoint): IPoint {
