...

{
  ...
  printf 'an oversight.\n\n'
  printf -- '- **Blocking the npm publish when a gate fails** - NOT PERFORMED, and this is\n'
  printf '  the most important line in this document. This workflow and\n'
  printf '  `.github/workflows/publish-sdk.yml` both trigger on the same `sdk-v*` tag.\n'
  printf '  GitHub starts them in parallel and there is no `needs:` or `workflow_run`\n'
  printf '  dependency between them, so if any gate reported above fails, `npm publish`\n'
  printf '  has already run or is running alongside it. A failure here withholds THIS\n'
  printf '  EVIDENCE PACKAGE - it does not withhold the package on the registry, and it\n'
  printf '  cannot recall it. An absent release page is indistinguishable from a\n'
  printf '  repository that simply does not publish releases, which is what the three\n'
  printf '  tags before this one looked like.\n'
  printf '  So read every result above as a post-hoc statement about what shipped, not\n'
  printf '  as a precondition of shipping. Making it a precondition requires moving the\n'
  printf '  publish into this workflow behind `needs:`, which also means repointing the\n'
  printf '  npm Trusted Publishing policy at this workflow file.\n'
