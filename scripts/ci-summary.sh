...

{
  ...
  printf '  review** - NOT PERFORMED ON THIS TAG. They live in\n'
  printf '  `.github/workflows/hardened-ci.yml`, which triggers on pushes to `main`, on\n'
  printf '  pull requests, and on tags matching `v*`. A `sdk-v*` tag does not match that\n'
  printf '  pattern, so those gates did not run for this release; they ran when the\n'
  printf '  commit landed on `main`. Dependency review is additionally pull-request only.\n\n'
