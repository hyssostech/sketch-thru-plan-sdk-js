...
export interface IStpConnector {
   ...
   *
   * Implementations that are already connected should UPDATE the existing registration with
   * the supplied `solvables` rather than opening a second connection, and return the
   * sessionId in force after that update.
   */
  connect(
    serviceName: string,
    solvables: string[],
