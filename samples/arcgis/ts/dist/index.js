(function () {
    ...
    async function start() {
        ...
        if (latParm && lonParm)
            mapCenter = { lat: parseFloat(latParm), lon: parseFloat(lonParm) };
        ...
        stpsdk.onSymbolAdded = (alternates, isUndo) => {
            try {
                const gj = alternates[0].asGeoJSON();
                map.addFeature(gj);
            }
            catch (e) {
        ...
        stpsdk.onTaskOrgAdded = (taskOrg) => { log('Task Org added: ' + taskOrg.name, 'Info'); };
        stpsdk.onTaskOrgModified = (poid, taskOrg) => { log('Task Org modified: ' + poid + ' ' + taskOrg.name, 'Info'); };
        stpsdk.onTaskOrgDeleted = (poid) => { log('Task Org deleted: ' + poid, 'Info'); };
        stpsdk.onTaskOrgUnitAdded = (toUnit) => { log('Task Org Unit added: ' + toUnit.fullDescription, 'Info'); };
        stpsdk.onTaskOrgUnitModified = (poid, toUnit) => { log('Task Org Unit modified: ' + poid + ' ' + toUnit.fullDescription, 'Info'); };
        stpsdk.onTaskOrgUnitDeleted = (poid) => { log('Task Org Unit deleted: ' + poid, 'Info'); };
        stpsdk.onTaskOrgRelationshipAdded = (toRel) => { log('Task Org Relationship added: ' + toRel.poid, 'Info'); };
        stpsdk.onTaskOrgRelationshipModified = (poid) => { log('Task Org Relationship modified: ' + poid, 'Info'); };
        stpsdk.onTaskOrgRelationshipDeleted = (poid) => { log('Task Org Relationship deleted: ' + poid, 'Info'); };
        ...
        stpsdk.onTaskAdded = (_poid, alternates) => { log('Task added: ' + alternates[0].description, 'Info'); };
        stpsdk.onTaskModified = (poid, alternates) => { log('Task modified: ' + poid + ' ' + alternates[0].description, 'Info'); };
        stpsdk.onTaskDeleted = (poid) => { log('Task removed: ' + poid, 'Info'); };
        stpsdk.onSymbolEdited = (operation, location) => { log('Symbol edit operation: ' + operation + ' gesture:' + location.shape, 'Info'); };
        stpsdk.onMapOperation = (operation, location) => { log('Map operation: ' + operation + ' gesture:' + location.shape, 'Info'); };
        stpsdk.onCommand = (operation, location) => { log('Command: ' + operation + ' gesture:' + location.shape, 'Info'); map.addPoly(location.coords); };
        stpsdk.onInkProcessed = () => { map.clearInk(); };
        stpsdk.onSpeechRecognized = (phrases) => { const speech = (phrases && phrases.length > 0) ? phrases[0] : ''; log(speech); };
        stpsdk.onStpMessage = (msg, level) => { log(msg, level, true); };
        const buttonNew = document.getElementById('new');
        buttonNew.onclick = async () => { try {
        ...
        roleSelect.onchange = async () => {
            try {
                ...
                const roleMap = { s2: StpSDK.StpRole.s2, s3: StpSDK.StpRole.s3, s4: StpSDK.StpRole.s4, fso: StpSDK.StpRole.fso, eng: StpSDK.StpRole.eng };
        ...
        buttonExport.onclick = async () => {
            try {
                if (await stpsdk.hasActiveScenario()) {
                    ...
                    const typeArg = (typesSelect.value === 'Initialization' ? 'initialization' : 'order');
                    const affArg = (affilSelect.value === 'friend' ? 'friend' : (affilSelect.value === 'hostile' ? 'hostile' : 'all'));
        ...
        c2simProxy.onSymbolReport = (poid, symbol) => { map.removeFeature(poid); try {
            const gj = symbol.asGeoJSON();
            map.addFeature(gj);
        }
        catch (e) {
    ...
    function buildInfo(symbol) {
        if (!symbol || !symbol.location || !symbol.location.centroid)
            return null;
        let contentString = '<h3 id="firstHeading" class="firstHeading">' + (symbol.fullDescription ?? symbol.description ?? 'Symbol') + '</h3>' +
            '<table>' +
